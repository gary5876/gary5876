...
동아리 모집·지원자 관리 서비스입니다. 카카오테크캠퍼스에서 3명이 9개월째 만들어 지금도 운영하고 있습니다. 저는 지원서 제출 도메인의 API와 DB 모델링을 맡았고 통계·공지·이메일 알림은 처음부터 끝까지 혼자 만들었습니다. 지원서 목록 조회가 느려서 join fetch와 프로젝션으로 쿼리를 줄였고, 이메일 발송은 이벤트로 분리해 재시도 가능한 실패와 아닌 실패를 나눠 처리했습니다. 장애를 바로 보려고 Prometheus·Grafana·Loki 모니터링을 팀에서 직접 붙였고, 테스트는 계층별로 290개를 쌓았습니다.
